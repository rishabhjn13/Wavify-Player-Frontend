...
export default function PlayerBar() {
    ...
    const {
        ...
        repeat, setRepeat,
        playing,
        currentSong, progressPercent
    } = useAudioPlayer();
    const { rightPanel, setRightPanel } = useUIStateContext();
        ...
        <div>
            <div className="player-bar player-bar-desktop" style={{
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 14, minWidth: 0 }}>
                    ...
                    <div style={{ minWidth: 0 }}>
                        ...
                        <div style={{ fontSize: 12, color: "#5a507a", marginTop: 1 }}>{currentSong?.artist} · {currentSong?.album}</div>
                    </div>
                    <button className={`iBtn${liked.has(currentSong?.song_id) ? " heart-lit" : ""}`} onClick={() => toggleLike(currentSong)} style={{ marginLeft: 4, flexShrink: 0 }}>
                        <svg width="18" height="18" viewBox="0 0 24 24" fill={liked.has(currentSong?.song_id) ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2">
                ...
                <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4, minWidth: 420 }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 10, width: "100%" }}>
                        ...
                        <input type="range" className="pbar" min={0} max={currentSong?.duration} value={progress}
                            onChange={e => {
                                const val = Number(e.target.value);
                                setProgress(val);
                                audioRef.current.currentTime = val;
                            }}
                            style={{ background: `linear-gradient(to right, #a78bfa ${progressPercent}%, #1a1635 ${progressPercent}%)` }}
                        />
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 10 }}>
                    ...
                    <input type="range" className="vbar" min={0} max={100} value={volume}
            ...
            <div className="player-bar player-bar-mobile">
                ...
                <div className="mobile-mini-player">
                    ...
                    <button className={`iBtn${liked.has(currentSong?.song_id) ? " heart-lit" : ""}`} onClick={() => toggleLike(currentSong)}>
                        <svg width="18" height="18" viewBox="0 0 24 24" fill={liked.has(currentSong?.song_id) ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z" /></svg>
                    </button>
